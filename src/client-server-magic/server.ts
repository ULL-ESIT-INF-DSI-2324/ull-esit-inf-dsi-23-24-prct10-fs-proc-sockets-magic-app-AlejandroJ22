  ...
  connection.on("data", (dataJSON) => {
    ...
    switch (action) {
      case "add":
        ...
        collection.addCard(newCard, (successMessage, errorMessage) => {
          const response = errorMessage ? errorMessage : successMessage;
          connection.write(JSON.stringify({ message: response }));
        });
        break;
      case "update":
        const updatedCard = createCardFromData(data);
        collection.updateCard(updatedCard, (successMessage, errorMessage) => {
          const response = errorMessage ? errorMessage : successMessage;
          connection.write(JSON.stringify({ message: response }));
        });
        break;
      ...
      case "list":
        ...
        break;
      case "remove":
        collection.removeCard(data.id, (successMessage, errorMessage) => {
          const response = errorMessage ? errorMessage : successMessage;
          connection.write(JSON.stringify({ message: response }));
        });
        break;
